...
2. **Apply it on every anchor path.** When the exponent is pinned, ignore the "same quantity anchors" shortcut for the quantity dimension: price the size curve at a single reference quantity and then scale by `(units / refQty)^e`. So the number moves whenever the exponent moves, on every path except an exact validated/anchor match (those stay verbatim by design).
